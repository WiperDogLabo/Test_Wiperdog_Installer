#!/bin/bash
#
# Test case for Wiperdog installer with none option
#
	if [[ "$1" == "" ]]; then
	...
	triggerDir="$wiperdogPath/var/job"
	jobClassDir="$wiperdogPath/var/job"
	jobInstDir="$wiperdogPath/var/job"
	mongoDb="somehost"
	mongoPort=9999
...
expect <<- DONE	
	...
	spawn $javaCommand -jar $installerJar
	expect "* -ni option"
	send " \r"
	#Ask for install wiperdog in current folder
