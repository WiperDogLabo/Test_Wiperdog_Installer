...
# WITH some other option (-j, -jd, -td, -id, -cd, -mp)
...
# ############################################################################
	...
	# ========= CASE 3 =========
	echo  ">>>>> CASE 3: Test installer with  some option out of -d option <<<<<"
...
expect <<- DONE	
	...
	spawn $javaCommand -jar $installerJar -d $wiperdogPath -j $nettyPort -jd $jobDir -cd $jobClassDir -td $triggerDir -id $jobInstDir -mp $policyEmail
	expect "* -ni option"
	send " \r"
